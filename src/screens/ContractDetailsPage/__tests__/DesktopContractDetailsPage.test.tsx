...
jest.mock(
    ...
    () => ({
        OrderDetails: function OrderDetails() {
            return <div data-testid="order-details">Order Details</div>;
        },
        EntryExitDetails: function EntryExitDetails() {
            return <div data-testid="entry-exit-details">Entry Exit Details</div>;
        },
    }),
    { virtual: true }
);
...
describe("DesktopContractDetailsPage", () => {
    ...
    it("shows error state when there is an error", () => {
        // Mock error state
        (useContractDetailsStream as jest.Mock).mockReturnValue({
            loading: false,
            error: "Failed to stream contract details",
        });
    ...
    it("renders contract card with mobile variant", () => {
        ...
        expect(screen.getByTestId("contract-card")).toHaveTextContent(
            "Contract Card: Rise - Volatility 100 (1s) Index - mobile"
        );
    });
});
