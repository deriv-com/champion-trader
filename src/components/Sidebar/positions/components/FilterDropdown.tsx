...
export const FilterDropdown: FC<FilterDropdownProps> = ({
  ...
  return (
    <div className="relative w-[50%]" ref={dropdownRef} onMouseDown={(event) => event.stopPropagation()}>
      ...
      {dropdownOpen && (
        <ul className={`absolute text-sm left-0 w-full border rounded-lg shadow-md mt-1 ${isDarkMode ? "bg-sidebar text-white border-gray-700" : "bg-gray-100 text-black border-gray-300"}`} onMouseDown={(event) => event.stopPropagation()}>
          {isOpenTab ? (
            <>
              <li 
