...
const VerticalTabList: React.FC<BaseTabListProps> = ({
  ...
  return (
    <div className="w-28 bg-black dark:bg-black">
      {tabs.map(({ label, value }) => (
        <button
          key={value}
          onClick={() => onSelect(value)}
          className={`
            ...
            ${selectedValue === value
              ? "text-left dark:bg-transparent dark:text-black dark:border-white dark:scroll-select-item flex items-center justify-center snap-center cursor-pointer dark:bg-white dark:text-black border-r-2 dark:border-white dark:active:border-white"
              : "bg-white text-black text-gray-500 dark:bg-black dark:text-gray-400 dark:hover:text-white dark:hover:border-r-2 dark:hover:border-[var(--tab-border-hover-color)] hover:border-white border-r-2 border-transparent"
            }
          `}
        >
          {label}
