...
export const useProductConfig = () => {
  ...
  const applyConfig = useCallback((config: ProductConfigResponse) => {
  ...
  const fetchProductConfig = useCallback(async (product_type: string, instrument_id: string) => {
  ...
  }, [applyConfig, configCache, setConfigCache, setConfigError, setConfigLoading, setProductConfig, toast]);
